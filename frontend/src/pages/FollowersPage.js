  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log("Investors fetched:", investors);
                ...
                <div className="flex flex-col grow items-start pt-6 pr-5 pl-20 max-md:pl-5 max-md:max-w-full">
                    {investors.length > 0 ? (
                      ...
                      {investors.map((investor) => (
                          <div key={investor.id} className="flex flex-col p-6 mt-6 max-w-full rounded-lg bg-neutral-800 w-[870px] max-md:px-5">
                                      ...
                                      <div className="flex flex-col whitespace-nowrap">
                                          <div className="flex gap-1 pr-3.5 text-2xl font-semibold tracking-wide text-stone-100">
                                              <Link to={`/startupDetails/${investor.id}`}>
                                                  <div style={{ overflow: 'hidden', textOverflow: 'ellipsis' }} className="no-underline hover:underline">
                                                      {investor.name}
                                                  </div>
                                              </Link>
                                          </div>
